                    ...
                    <div class="card-body">
                        <div class="card-body ">
                            <form enctype="multipart/form-data" role="form" id="myform" method="POST"
                                action="{{ route('vendor_store_edit_post') }}">
                                @csrf
                                                ...
                                                <select name="vendor" class="custom-select" required>
                                                    ...
                                                    @foreach ($vendors as $vendor)
                                                        <option value="{{ $vendor->id }}"
                                                            @if ($store->first()->vendor->id == $vendor->id) selected @endif>
                                                            {{ $vendor->name }}</option>
                                                    @endforeach
                                                </select>
                                                @error('name')
                                ...
                                <h5 class="justify-content-center">Add More Store</h5>
                                <div class="row ">
                                    <div class="col-md-3  vendor_store">
                                        <label for="">Store Name</label>
                                    </div>
                                    <div class="col-md-3  vendor_store">
                                        <label for="">Category Type</label>
                                    </div>
                                    <div class="col-md-2  vendor_store">
                                ...
                                <div id="container" class="mb-4">
                                    ...
                                    @foreach ($store as $key => $store_vs)
                                        <input type="hidden" name="id[]" value="{{ $store_vs->id }}">
                                        <div class="row">
                                            <div class="col-md-3 vendor_store add_more">
                                                <select name="store_id[]" class="custom-select" required>
                                                    <option value="">---- Select Store ----</option>
                                                    @foreach ($stores as $store)
                                                        <option value="{{ $store->id }}"
                                                            @if ($store_vs->store->id == $store->id) selected @endif>
                                                            {{ $store->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                                ...
                                                <select name="category_id[]" class="custom-select" required>
                                                    ...
                                                    @foreach ($categories as $category)
                                                        <option value="{{ $category->id }}"
                                                            @if ($store_vs->category->id == $category->id) selected @endif>
                                                            {{ $category->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="col-md-2 vendor_store add_more">
                                                <input type="number" name="rent[]" class="form-control" placeholder="Rent"
                                                    value="{{ $store_vs->rent }}" required>
                                            </div>
                                            <div class="col-md-2 vendor_store add_more">
                                                <input type="number" name="percent[]" class="form-control"
                                                    placeholder="Percentage" value="{{ $store_vs->percent }}" required>
                                            </div>
                                            ...
                                            <div class="col-md-2 vendor_store add_more">
                                                ...
                                                @else
                                                    <a href="{{ route('vendor_store_remove', $store_vs->id) }}"
